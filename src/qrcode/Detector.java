...
//	    if(testParam->do1){
	    ...
	    Mat imagePoint = getImagePoint(src);
         ...
         System.out.println(finalpoint.size());
         for(int i=0;i<finalpoint.size();i++) {
        	 pointset.add(finalpoint.get(i));

        	
         }
         for(Point list:pointset) {
        	 Imgproc.circle(backg, new Point(list.x,list.y), 10, new Scalar(0,0,255),2);
        	 System.out.println("x坐标"+list.x+"y坐标"+list.y);
         }
         /*!!!注意调用此类的方法之前确保有三个点的坐标，保持竖直放置 */
         Locate locate=new Locate();
         locate.setPointset(pointset);
         locate.computeRect();
	    return backg;
	}
	void initMat() {
    ...
    Mat getImagePoint(Mat src) {
       ...
       Imgproc.adaptiveThreshold(graySrc,binarySrc,255,Imgproc.ADAPTIVE_THRESH_MEAN_C,Imgproc.THRESH_BINARY,blockSize,10);
    ...
    float centerRecognize(Mat src1,boolean rowORcol) {
        ...
        if(pattern.isEmpty()){
            ...
            for(int i=0;i<3;i++) {//去除最大的三个点
            	...
            	System.out.println("di"+i+"为"+pattern.get(i).count);
            }
